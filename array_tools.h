...
template <class T>
const unsigned int get_pos_climbing(T* buffer, const unsigned int size, const unsigned int pos);
template <class T>
const unsigned int get_pos_descending(T* buffer, const unsigned int size, const unsigned int pos);
...
const int get_nth_largest_elem_idx(T* buffer, const unsigned int size, const unsigned int nth)
{
    ...
    if (nth < (size / 2)) {
        return get_pos_climbing<T>(buffer, size, nth);
    } else {
        return get_pos_descending<T>(buffer, size, nth);
    }
}
